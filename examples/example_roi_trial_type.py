...
def mock_task_logic() -> AindForceForagingTaskLogic:
    ...
    updaters: Dict[str, task_logic.NumericalUpdater] = {}

    operation_control = task_logic.OperationControl(
        force=task_logic.ForceOperationControl(
            press_mode=task_logic.PressMode.SINGLE_AVERAGE, left_index=1, right_index=2
        )
    )
    ...
    return AindForceForagingTaskLogic(
        task_parameters=AindForceForagingTaskParameters(
            rng_seed=None, updaters=updaters, operation_control=operation_control, environment=agent_environment
        )
    )
